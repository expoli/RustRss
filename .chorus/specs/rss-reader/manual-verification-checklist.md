...
- [ ] 外部图片离线、原生Wayland/其它平台、逐篇阅读未验。报告：`2026-09-23-theme-preview/search-offline.md`。
  → **部分已被取代**（2026-09-24）：外部图片离线与逐篇阅读**已补验**（缩略图四探针 + `2026-09-24-local-acceptance-closeout/` 的离线计数 16，见 §27 与本清单 §24.21 注记）；**仍 open**：原生 Wayland 输入、其它平台。
...
- [x] 时延实测量与口径标注：宽泛查询冷 **265ms**、热 **263ms**（200 行封顶页，断言为 `==200` 而非 `>0`）；单字 CJK 热 **255ms**（多轮实测区间：冷 265–288ms / 热 263–267ms / 单字 254–255ms；数字以 `search-e2e-results.json` 为准，每轮含 `binary` + `binary_mtime`）。库文件每轮新拷贝 → 首查是 **SQLite 连接级冷**（**OS 页缓存不保证冷**，已按保守方向标注）。
...
- [x] **运行时双语证据（已在含本改动的 release 产物上重生：21:25 构建）**：`scripts/verify-error-localization.py`（本地故障夹具 + 隔离实例 + zh-CN / en 各启动一次）exit=0 —— 429（限流文案）、503、404、非 feed（`parse_error`）在侧栏 tooltip 里**两种语言文案不同**，且库里的 `last_status` 码先被断言为非 ok。
- [ ] **未验（保留 open，后续不得静默勾掉）**：① 非抓取类 Rust 文案（全文抓取 / AI / 命令层）仍是中文，需返回错误码由界面翻译；② fcitx5/ibus 候选窗与合成输入；③ DNS/TLS 原始后端详情在界面上的呈现深度；④ 其它平台。
...
## 33. 许可切为 AGPL-3.0-or-later 的界面与产物口径（2026-09-25）

- [x] **仓库许可**：根 `LICENSE` 换成 GNU AGPL-3.0 官方全文（gnu.org 版本；与 SPDX 收录文本归一化后仅 `fsf.org` 链接 http→https 一处差异）；workspace `license = "AGPL-3.0-or-later"` 由三个 crate 继承（`cargo metadata` 逐个核对）；GitHub 许可识别由 `Apache-2.0` 变为 `AGPL-3.0`。配套 `LICENSE-COMMERCIAL.md`（商业授权入口）/ `TRADEMARK.md` / `CONTRIBUTING.md`（贡献授权 + 再许可权）；历史 MIT / Apache 文本保留在 `LICENSES/`，边界提交 `cfaf0dc`（已发出的授权不可撤销）。决策记录见 `2026-09-24-license-policy/`。
- [x] **界面「源码」入口**：设置 → 关于 新增「源码 / 打开仓库」行，文案走 `ui/i18n.js` 双语 key（zh / en 各 **519** 个 key，启动自检 `i18n selftest ok (keys=519)`）。运行时验收 `python3 scripts/verify-about-source.py` **exit=0**：隔离实例 + 真实 WebView，DOM 断言行/按钮在 `#pane-general` 内且可见、en 与 zh-CN 两种文案随 locale 切换、**点击前打开器无调用、点击后 PATH 前置的 `xdg-open` 桩收到 `https://github.com/expoli/RustRss`**（因果明确）。证据 `2026-09-24-license-policy/about-source-results.json`。
- [x] **产物口径（已端到端验到）**：`nightly.yml` / `release.yml` 的 release notes 明写「本产物按 AGPL-3.0-or-later 授权…对应源码：<仓库>/tree/<tag>」。实机：`18666e5` 的 nightly run **success**，release notes 实际输出含该两行且 `${{ github.sha }}` 正确插进 URL（`…/tree/18666e51c3979654c1a39d88fbf95c2d03191229`），发布目标从 `3c203de`（MIT 期）换成 `18666e5`，旧产物被 `delete --cleanup-tag` 替换（当前下载量 0）。CI 侧：`0cf53d6` 与 `18666e5` 的 ci / nightly 均 **success**（`18cf3e0` / `cfaf0dc` 上的失败未复现）。
