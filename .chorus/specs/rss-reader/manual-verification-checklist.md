...
- [x] **产物口径**：`nightly.yml` / `release.yml` 的 release notes 明写「本产物按 AGPL-3.0-or-later 授权…对应源码：<仓库>/tree/<tag>」，即「对目标码明确指向对应源码」；两个 workflow 改动后 `yaml.safe_load` 解析通过。
...
- [ ] **未做（用户决定）**：不改写历史（241 个提交与 MIT 期历史保留）；`nightly` 旧产物（`3c203de` 构建，下载量 0）的替换取决于 nightly 构建能否成功。
