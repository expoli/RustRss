...
- [ ] 输入网站首页 URL 时能自动发现 feed（解析 `<link rel="alternate">`）；输入 feed URL 直接订阅成功
...
- [ ] 已读/未读、星标状态即时反映到列表与计数；「全部标记已读」可限定作用域（当前源 / 当前视图）
