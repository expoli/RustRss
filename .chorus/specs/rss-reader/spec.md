...
- [x] 条目列表渐进加载：滚到底自动续下一批（每批 200 条，keyset 游标分页：游标 = 上一批末行的 `(sortkey, id)`），未读 / 全部 / 星标 / 稍后读 / 单源视图均支持，加载完全部匹配条目后停止（无重复、无跳条）；续页只 append 新行、不重建已有 DOM；后台刷新（多页已加载时）只 prepend 新条目并按 `scrollHeight` 增量补偿 `scrollTop`，会话内已读条目不回插，续页游标/`exhausted`/哨兵不受影响；搜索仍为一次性 200 条
