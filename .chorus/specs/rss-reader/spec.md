...
- [ ] 全文搜索（标题 + 正文）可返回结果并按相关度/时间排序，10k 篇文章库内查询响应可感知为即时
  ...
  → 2026-09-24 **release 口径端到端实测**（`scripts/verify-search-e2e.py`：10k 生产形状库、真实 WebView 键入 → 列表渲染）：宽泛查询**冷 268ms / 热 267ms**（200 行封顶页，断言等于 200 行）、单字 CJK 热 254ms。**未达「可感知为即时」**，故本项保持 open——core 口径（16–18ms / 65–77ms）不代表 UI 成本；证据 `2026-09-24-local-acceptance-closeout/search-e2e-results.json`。原生 Wayland 会话下的搜索输入未验（需用户在场）。输入法：探针注入的是真实键盘事件（isTrusted），fcitx5/ibus 候选窗不在其范围。
...
- [x] 数据库 schema 带版本号与**首发基线**（迁移链已压平成一份基线）；**不保开发库**：旧链任意版本的库（含 `user_version` 恰为 1 的 v1 冻结库）按 `application_id` 魔数识别，并在**任何写入之前**被拒绝（`durability::legacy_v1_frozen_db_is_refused_at_detect_level` 与 `legacy_frozen_db_is_refused_before_any_write`）；桌面端给出双语拒绝说明 + 只读导出 OPML + 退出，MCP 二进制非零退出；见 [变更目录](2026-09-24-release-schema-baseline/prd.md) 与 [清单第 25 节](manual-verification-checklist.md#25-首发-schema-基线压平与老库拒绝2026-09-24-release-schema-baseline)
...
- [x] 真实订阅列表的缩略图元数据来源与展示：公网真 feed（`github.blog/feed/`，2026-09-24 @10:16:43Z）解析后 4 条条目回填 `thumbnail_url`（SQLite 回读）并在生产 WebView 渲染，见 [结果 JSON](2026-09-24-thumbnail-egress-policy/thumbnail-public-feed-results.json) 与截图 `thumbnail-public-feed.png`；**公网热链策略仍未测**（用本地 403 夹具替代，见 [验证清单 §24.24](manual-verification-checklist.md)）。
...
- [ ] T7 第三方GUI客户端、空状态专项、其它平台/读屏器/分数缩放及用户侧独立聚合评审。
  → 2026-09-24 更新（用户侧独立聚合评审已发生）：**round 1 判 FAIL**（1 BLOCKER + 4 NOTE，评论 `4d2c4ff1`）——BLOCKER 是把「空状态夹具矩阵」错误归类为环境依赖、而它本机可跑。修复 `178757a`：补跑空状态矩阵 36 格（`2026-09-23-theme-preview/empty-state-matrix.md`，含变异校验）；同时发现**原生截帧只随主题刷新、不随场景刷新**（同格三场景逐字节相同，清单 §31），故**逐场景像素证据仍缺**；MCP 回归因后续批次的坐标点击失效已改键盘驱动并重跑通过；`tracking.json` 里无凭据的「用户报告独立评审通过」已被替换为本次裁决引用。（round 2 结果：**PASS WITH NOTES**，`4310c308`，0 BLOCKER）**读屏器已于 2026-09-25 本机验通**（清单 §32 + `screen-reader-results.json`：AT-SPI 客户端读到 168 节点、13 条与库标题逐字匹配，三态之 ①）；**仍 open**：真实桌面下的语音/盲文输出与动态播报、其它合成器/跨屏缩放、第三方 GUI 客户端、Windows/macOS/GNOME-Wayland、企业证书/认证代理。（round 2 结果：**PASS WITH NOTES**，评论 `4310c308`）round 2（评论 `4310c308`）**PASS WITH NOTES**：BLOCKER 已修（含变异校验）、N1/N2/N3 已修、N4 以重跑带指纹的 `mcp-theme-results.json` 收口；无新增 BLOCKER/NOTE。（另记非阻塞残留：矩阵探针 stdout 曾硬编码 `pixel_checks: 24`，已改为实际值。）**仍 open**：真实桌面下的语音/盲文输出与动态播报（读屏器已在 headless 下验通：AT-SPI 客户端读到 168 节点、其中 13 条与库标题逐字匹配，见清单 §32）、原生弹窗键盘/深色路径、按场景的像素证据（需真实合成器会话）、Windows/macOS/GNOME-Wayland、第三方 GUI MCP 客户端。（**已验**：空/错误状态双语与失败保留缓存见 [空态报告](2026-09-23-theme-preview/empty-error-states.md)；分数缩放 KDE Wayland 125%/150% 与 Xvfb 100%/200% 见 [原生验收](2026-09-23-theme-preview/native-settings.md)。**未验**：第三方 GUI 客户端（仅 Codex CLI 收图）、读屏器、GNOME/无 XWayland、**用户侧独立聚合评审**（T7 报告明确 fresh reviewer 由用户另派）→ 保留 open）
