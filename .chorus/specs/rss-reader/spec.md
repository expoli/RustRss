...
- [ ] 从终端运行时日志镜像一份到 stderr（stdout 是终端即开；`RUSTSS_LOG_STDOUT=0/1` 强制覆盖）；镜像行为与文件行逐字节一致，非终端启动无额外输出（见 2026-09-23-log-tty）
...
- [x] 同一条查询在 MCP 与 GUI 中返回一致结果（同一 core 数据层）——⚠ 现状：MCP 列表**继承**界面排序/隐藏已读设置；「默认口径固定为最新在前 + 不隐藏已读」属 2026-09-23-mcp-write 待交付项
