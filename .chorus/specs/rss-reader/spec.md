...
- [ ] 外部链接仅允许 http/https，且打开动作不经 shell 解析（Windows 下 URL 中的 `&` 等元字符无法注入命令；见 2026-09-22-audit-remediation-1）
...
- [ ] 单源抓取受体积上限保护（8 MiB）：超限即中止下载，条目不被写入、ETag 不被覆盖（失败态按既有失败路径记录 last_status/last_error，与其它抓取失败一致），并给出可读失败原因（见 2026-09-22-audit-remediation-1）
...
- [ ] 每次启动创建独立日志文件（`logs/` 下按启动时间命名），行内含时间戳/级别/模块，覆盖 Rust 侧关键事件与 UI 诊断行（`[ui]`）
- [ ] 崩溃（panic）写入日志文件（含 payload 与位置），不改变既有 stderr 行为
- [ ] 日志保留最近 20 个文件且总量 ≤50MB，启动时自动清理最旧的
- [ ] 设置 → 关于 提供「打开日志目录」（系统文件管理器打开，路径不经 shell 解析）
- [ ] 日志级别可在设置中切换（info/debug，默认 info，切换即时生效）
- [ ] 日志中不出现 AI API key / MCP token 等敏感信息
- [ ] 日志初始化失败不阻断应用启动（降级为无日志继续运行）
...
- [ ] 渲染层有最小 CSP 纵深（`script-src 'self'` 等）：主流程零 `securitypolicyviolation`，文章图片/AI 面板/设置页渲染正常（见 2026-09-22-audit-remediation-1）
...
- [ ] 初始化失败时窗口仍可关闭（最小事件绑定集无条件生效，见 2026-09-22-audit-remediation-1）
- [ ] 阅读中标记已读/星标/稍后读不重置正文滚动位置、不清空 AI 面板内容（行级 patch 而非整区重建，见 2026-09-22-audit-remediation-1）
