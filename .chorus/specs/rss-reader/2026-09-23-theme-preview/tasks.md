...
先做 T1，尽早验证最大不确定性；T2 随后逐项实施，按用户继续 MCP 闭环的优先级，T3 后先执行 T5→T6，再补 T4 与 T7；T4 设置重组已完成 Linux Xvfb 验收；T7 已按用户指示执行，证据与 open item 见 [T7 报告](t7-aggregate.md)；独立评审由用户另派。
...
- [x] T4 设置七类、外观编辑/历史恢复、Aa 共用排版；Linux Xvfb 验证见 [T4 报告](t4-settings.md)。
- [ ] 真实列表缩略图数据来源；当前只验证已有元素的样式开关。
...
## T7 执行与剩余待办（2026-09-24）

- [x] 当前 KDE 原生 Wayland：2026-09-24 用户在场真实点击取消通过，窗口销毁/正式主题不变/正文节点及段落偏移保留；见 [现场证据](t7-wayland-input-results.json)。
- [x] 30 分钟墙钟长测：每两分钟捕获保持活动会话，验证 30 分钟绝对上限；另测 10 分钟空闲回收。记录进程 RSS、窗口数、数据库/日志/临时目录大小和清前清后 df；明确热态测量，不冒充冷盘性能。
- [ ] 三预设×明暗×三场景完整回归、原生选择弹层/字体建议的明暗与键盘体验、读屏器与分数缩放、第三方 MCP 图片客户端、Windows/macOS/GNOME 等能力矩阵。

T7 已获用户授权在执行汇总后 commit + push；不创建 Chorus proposal/task，不派发或自任独立聚合评审。详见 [T7 执行报告](t7-aggregate.md)。

- [x] T4→T6 ID/资源/CSS 契约回归已修复并带复现测试；完整组件矩阵117图、MCP产品回归25图。
- [x] Wayland真实点击组合项已在用户在场时补验关闭；isTrusted=true，不依赖自动portal授权。
