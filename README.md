...
- [x] 键盘导航：`j`/`k` 上下 · `Enter` 打开 · `u` 未读切换 · `s` 星标 · `t` 标签 · `r` 刷新 · `/` 搜索 · `Esc` 清除 · `g`/`G` 首尾
