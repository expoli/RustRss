...
当前版本**不保开发库**：数据库用 SQLite 原生的 `PRAGMA application_id`（基线写 `0x52535331`）标识自己，`user_version = 1` 只表示首发基线。

- **旧开发库 / 其它程序的 sqlite 文件**（缺少应用标识）打开时被**拒绝**，且拒绝发生在**任何写入之前**（不改库头、不建 WAL 边车）；`user_version` 与基线不一致的库同样被拒。
- **桌面端**不会只留一行日志：它用内存库把界面撑起来，显示双语的拒绝说明，只提供两个动作——**导出 OPML**（只读，不写旧库一个字节）与**退出**；托盘、MCP 服务与定时刷新都不会启动。
- **独立 MCP 二进制**命中同一错误时以非零退出码 + 可读错误结束，不会自建新库。
- **备份恢复**口径同步收紧：只接受带应用标识且 `user_version` 等于基线的备份；旧开发库、空库、文本文件与更高版本的备份都会被拒绝并给出可读原因。
- 迁移链在首发前已压平成一份基线（`crates/rustrss-core/src/store/schema.rs`），发布后只能追加新迁移。
